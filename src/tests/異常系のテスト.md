...
このテストで検証する異常は以下3系統。 [ppl-ai-file-upload.s3.amazonaws](
...
| 3  | 読み取り専用ファイル | 例外 → ロールバック   | [ppl-ai-file-upload.s3.amazonaws](

※ 本番コードは**変更しない**  
※ テスト環境のみ細工する [ppl-ai-file-upload.s3.amazonaws](
...
- **既存ファイル衝突で即停止**
- 1ファイルも移動されない
- ログに `ERROR:` 記録あり [ppl-ai-file-upload.s3.amazonaws](

## 異常② 途中例外
- 数ファイル移動後に例外
- **全ファイルが元の場所に戻る**
- 作成フォルダは削除される [ppl-ai-file-upload.s3.amazonaws](

## 異常③ 読み取り専用
- `Move-Item` 失敗
- **ロールバック完全実行**
- ReadOnly属性は保持されたまま [ppl-ai-file-upload.s3.amazonaws](
...
- **「たまたま動いた」を否定できる**
- ロールバック設計が正しいことを証明
- 本番事故時の再現テストにも使える [ppl-ai-file-upload.s3.amazonaws](
